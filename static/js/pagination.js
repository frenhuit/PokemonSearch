jQuery.fn.pagination = function (maxentries, opts) {
    opts = jQuery.extend({
        ...
        prev_text: "Previous page",   //上一页的文字
        next_text: "Next page",	   //下一页的文字
    ...
    return this.each(function () {
        ...
        function drawLinks() {
            ...
            if (opts.display_msg) {
                if (!maxentries) {
                    panel
                        .append('<div class="pxofy">No data available</div>');
                } else {
                    panel
                        .append('<div class="pxofy">Showing No.&nbsp;'
                            + ((current_page * opts.items_per_page) + 1)
                            + '&nbsp; to No.&nbsp;'
                            + (((current_page + 1) * opts.items_per_page) > maxentries
                                ? maxentries
                                : ((current_page + 1) * opts.items_per_page))
                            + '&nbsp;，Total &nbsp;' + maxentries + '&nbsp;</div>');
                }
            }
            //设置跳到第几页
            if (opts.setPageNo) {
                panel.append("<div class='goto'><span class='text'>Go to No.</span><input type='text'/><span class='page'>Page</span><button type='button' class='ue-button long2'>确定</button></div>");
            }
        }
        ...
        if (maxentries == 0) {
            panel.append('<span class="current prev">' + opts.prev_text + '</span><span class="current next">' + opts.next_text + '</span><div class="pxofy">No data available</div>');
        } else {
            drawLinks();
        }
